...
import { Resource, Mapping, Color } from '../common.js';
...
export class CanvasView {
    ...
    drawChit(chit, pt) {
        ...
        this.drawText(Mapping.diceDistroText[chit], point(pt.x, pt.y + 5), clr);
    }
    ...
    drawHelper() {
        ...
        for(const pt of points) {
            ...
            this.drawSettlement(points[Mapping.nodeIndex[name]], 'white');
    ...
    drawDice(a, b) {
        ...
        let txt = this.ctx.measureText(Mapping.dice[a]);
        this.ctx.fillText(Mapping.dice[a], x, y);
        this.ctx.fillText(Mapping.dice[b], x + 25, y);
    ...
    edgeCenter(edge, points) {
        const [i, j] = Mapping.edgeIndices[edge];
    ...
    renderHex(hex) {
        ...
        if(hex.isWater) {
            const [a, b] = [
                dims.edges[Mapping.nodeIndex.f].center,
                dims.edges[Mapping.nodeIndex.c].center
            ];
    ...
    renderHarbor(harbor, hex) {
        const dims = this.hexDims(hex);
        const edge = dims.edges[Mapping.nodeIndex[harbor.edge]];
        const prev = dims.edges[Mapping.prevNode[harbor.edge]];
        const next = dims.edges[Mapping.nextNode[harbor.edge]];
        const ctx = this.ctx;
        const N = 24;
    ...
    renderDice(roll) {
        const dice = (function() {
            if(roll >= 4 && roll <= 10) {
                return Utils.randomIndex(Mapping.diceCombos[roll]);
            }
            let half = roll / 2;
    ...
    renderSettlement(hex, vertex, color) {
        const dims = this.hexDims(hex);
        this.drawSettlement(dims.points[Mapping.nodeIndex[vertex]], color);
    }
    renderCity(hex, vertex, color) {
        const dims = this.hexDims(hex);
        this.drawCity(dims.points[Mapping.nodeIndex[vertex]], color);
    }
    renderRobber(hex) {
        const dims = this.hexDims(hex);
